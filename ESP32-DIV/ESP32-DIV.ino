...
void handleWiFiSubmenuButtons() {
    ...
    if (isButtonPressed(BTN_SELECT)) {
        ...
        if (current_submenu_index == NUM_SUBMENU_ITEMS - 1) {  // Back to Main Menu (last item)
            in_sub_menu = false;
            feature_active = false;
            feature_exit_requested = false;
            displayMenu();
        ...
        if (current_submenu_index >= 7 && current_submenu_index < NUM_SUBMENU_ITEMS - 1) {
            in_sub_menu = true;
            feature_active = true;
            feature_exit_requested = false;
    ...
    if (!feature_active) {
        ...
        for (int i = 0; i < active_submenu_size; i++) {
            ...
            if (x >= button_x1 && x <= button_x2 && y >= button_y1 && y <= button_y2) {
                ...
                if (current_submenu_index == NUM_SUBMENU_ITEMS - 1) {  // Back (last item)
                    in_sub_menu = false;
                    feature_active = false;
                    feature_exit_requested = false;
                    displayMenu();
                    handleButtons();
                    is_main_menu = false;
                } else if (current_submenu_index >= 7 && current_submenu_index < NUM_SUBMENU_ITEMS - 1) {  // bySaw tools
                    in_sub_menu = true;
                    feature_active = true;
                    feature_exit_requested = false;
                    switch (current_submenu_index) {
